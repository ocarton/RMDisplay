...
.controller('ARVEWTRCtrl', function ARVEWTRController($scope, $cookies, auth, $http, $location, store) {
    ...
    var timestampName = "extraction";
    var timezone = convertMinsToHrsMins(-new Date().getTimezoneOffset()); //-120 means UTC+02:00
    var updateWTRTS = new Date(0);
    var updateText = d3.select("#ARVEWTRChart").select("#updateWTRText");
    ...
    function getNode(tree, name) {
        if (tree.children) {
            tree.children.forEach(function (d) {
                if (d.name == name) { newGroupArray = newGroupArray.concat(d); }
    ...
    function loadData() {
        ...
        d3.json(fList[2], function (error, data2) {
            ...
            root[2] = data2;
            initNodeData(root[2]);
            updateWTRTS = getLastUpdateTS(data2[timestampName], updateWTRTS);

            d3.json(fList[3], function (error, data3) {
                if (error) throw error;
                root[3] = data3;
                initNodeData(root[3]);
                updateWTRTS = getLastUpdateTS(data3[timestampName], updateWTRTS);

                d3.json(fList[4], function (error, data4) {
                    if (error) throw error;
                    root[4] = data4;
                    initNodeData(root[4]);
                    updateWTRTS = getLastUpdateTS(data4[timestampName], updateWTRTS);

                    d3.json(fList[0], function (error, data0) {
                        if (error) throw error;
                        root[0] = data0;
                        initNodeData(root[0]);
                        updateWTRTS = getLastUpdateTS(data0[timestampName], updateWTRTS);
                    ...
                    d3.json(fList[1], function (error, data1) {
                        ...
                        root[1] = data1;
                        initNodeData(root[1]);
                        updateWTRTS = getLastUpdateTS(data1[timestampName], updateWTRTS);

                        console.log("WTR chart updated at " + updateWTRTS);
                        updateText.html(updateWTRTS.toLocaleString());
